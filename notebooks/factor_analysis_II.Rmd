...
cfa_agree_neuro = cfa(modelCode, data=bfi_agree_neuro)
summary(cfa_agree_neuro, standardized=T, rsq=T, fit=T, nd=2)
```

We can visualize it as well.

```{r bfi_agree_neuro_vis}
# requires lavaanPlot
# lavaanPlot::lavaanPlot('faplot', cfa_agree_neuro, coefs=T, stand=T)

# otherwise use psych, will show raw rather than standardized loadings
lavaan.diagram(cfa_agree_neuro, lr=F)
```
...
```{r cfa_fit}
glance(cfa_agree_neuro) %>% mutate_if(is.numeric, round, digits = 2)
```
